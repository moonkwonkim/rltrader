...
class PolicyLearner:
    ...
    def fit(
        self, num_epoches=1000, max_memory=20, balance=2000000,
        discount_factor=.99, start_epsilon=1., learning=True):
        ...
        for epoch in range(num_epoches):
            ...
            # 메모리 초기화 및 최초 샘플 생성
            memory_sample = [self._build_sample()]
            memory_action = []
            memory_reward = []
            memory_prob = []
            memory_pv = []
            ...
            while True:
                ...
                if learning and delayed_reward != -1:
                    # 배치 학습 데이터 크기
                    batch_size = min(batch_size, max_memory)
                    # 배치 학습 데이터 생성
                    x, y = self._get_batch(
                        memory, batch_size, discount_factor, max(delayed_reward, 0))
                    if len(x) > 0:
                        if delayed_reward > 0:
                            pos_learning_cnt += 1
                        else:
            ...
            logger.info("[Epoch %s/%s]\tEpsilon:%.4f\tLoss:%10.6f\t"
                        "#Expl.:%d/%d\t#Stocks:%d\t"
                        "#Buy:%d\t#Sell:%d\t#Hold:%d\t"
                        "PV:%s\tPOS:%s\tNEG:%s" % (
                            epoch_str, num_epoches, epsilon, loss,
                            exploration_cnt, itr_cnt, self.agent.num_stocks,
    ...
    def _get_batch(self, memory, batch_size, discount_factor, delayed_reward):
        ...
        for i, (sample, action, reward, next_sample) in enumerate(
                reversed(memory[-batch_size:])):
            x[i] = np.array(sample).reshape((-1, 1, self.num_features))
            for a in self.agent.ACTIONS:
                if a == action:
                    y[i, a] = delayed_reward
                else:
                    y[i, a] = 1 - delayed_reward
                if discount_factor > 0:
                    y[i, a] *= discount_factor ** i
        return x, y
    ...
    def trade(self):
        self.fit(num_epoches=1, learning=False)
